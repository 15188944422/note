...
public void test03(){
    ApplicationContext ctx=new AnnotationConfigApplicationContext(SpringConfig.class);
    Student student = (Student) ctx.getBean("wangwuStudent");
    System.out.println("使用JavaConfig创建的bean对象"+student);
}
```
...
@ImportResource 作用导入其他的xml配置文件， 等于 在xml `<import resources="其他配置文件"/>`
...
@Configuration
@ImportResource(value ={ "classpath:applicationContext.xml","classpath:beans.xml"})
@PropertySource(value = "classpath:config.properties")//引入配置文件
@ComponentScan(basePackages = "com.bjpowernode.vo")//@Component("tiger")注解所在的包名 Tiger 
public class SpringConfig {
}
```
...
## 2.3  注解的使用SpringBootApplication
...
默认扫描的包： @ComponentScan所在的类所在的包和子包。
    ...
    例如在Application类上有注解为@SpringBootApplication
    	它所在的包为package com.song
    	它下面的子包 以及类 都会被扫描到
    	package com.song.**
...
使用application.properties,   application.yml  如果配置文件这两个都有默认使用 properties这个配置文件
...
![](D:\Power\springboot\bjpowernode\笔记\images\多文件配置.png)



那怎么使用其中一个配置文件呢

需要在主配置文件(application.yml|properties)中设置
...
注意 使用ConfigurationProperties这个注解需要添加依赖

```xml
<!--处理有关ConfigurationProperties的元数据-->
<dependency>
...
8. 写application.properties文件
   ...
   spring.datasource.url=jdbc:mysql://localhost:3306/bjpowernode?useUnicode=true&characterEncoding=UTF-8&serverTimezone=GMT%2B8
...
public class MyRestController {
    ...
    /**
     * @PthVariable(路径变量):获取url中的数据
     *          属性: value:路径变量名
     *          位置: 放在控制器方法形参的前面
     *
     *      http://localhost:8080/myboot/student/1002
     *      {stuId}:定义路径变量,stuId定义变量
     */
...
<body>
    <div style="margin-left: 400px">
        ...
        <div th:each="lm:${listmap}"> lm 代表的是每个 map集合

            <div th:each="m:${lm}">  m才代表map集合中的元素
